      ...
      <div className="pt-28 pb-12 md:pt-36 md:pb-16">
        <div className="md:hidden flex items-center mb-8">
          ...
          <div className="flex items-center text-sm whitespace-nowrap min-w-0 ml-3">
            <span className="text-slate-500">{post.metadata.topicName}</span>
            <svg className="h-3 w-3 shrink-0 fill-slate-400 mx-2" viewBox="0 0 16 16">
              <path d="M6.6 13.4L5.2 12l4-4-4-4 1.4-1.4L12 8z"></path>
            </svg>
            <span className="text-slate-800 font-medium truncate">{post.metadata.title}</span>
          </div>
        ...
        <article>
          <div className="flex items-center mb-4">
            ...
            <span className="text-lg font-bold text-slate-800 ml-2.5">{post.metadata.topicName}</span>
          </div>
          <h1 className="h3 text-slate-800 mb-4">{post.metadata.title}</h1>
